...
class BaseService(Generic[ModelType, CreateSchemaType, UpdateSchemaType, RepositoryType], ABC):
    ...
    async def create(self, obj_data: CreateSchemaType) -> ModelType:
        ...
        async with get_async_db_session() as db:
            ...
            created_obj = await repository.create(obj_data)
            await self._after_create(created_obj, repository)
            return created_obj
    ...
    async def get(self, obj_id: int) -> ModelType | None:
        ...
        async with get_async_db_session() as db:
            repository = self.repository_class(db)
            return await repository.get(obj_id)
    ...
    async def update(self, obj_id: int, obj_data: UpdateSchemaType) -> ModelType | None:
        ...
        async with get_async_db_session() as db:
            ...
            updated_obj = await repository.update(obj_id, obj_data)
