# app/db/async_db/services/base.py
...
class BaseService(
    ...
    """
    Abstract base service encapsulating standardized business logic patterns (async version).

    This class provides a consistent interface for CRUD operations while enforcing:
        - Automatic database session management via async context manager,
    ...
    async def create(self, obj_data: CreateSchemaType) -> ReadSchemaType:
        """
        Create a new entity with optional pre- and post-creation business logic.

        The method acquires an async database session, invokes custom validation, delegates to the repository
        ...
        async with get_async_db_session() as db:
            ...
            orm_obj = await repository.create(obj_data.model_dump())
            await self._after_create(orm_obj, repository)
            await db.commit()
            return self.read_schema_class.model_validate(orm_obj)

    async def get(self, obj_id: int) -> ReadSchemaType | None:
        """
    ...
    async def update(self, obj_id: int, obj_data: UpdateSchemaType) -> ReadSchemaType | None:
        """
        ...
        async with get_async_db_session() as db:
            ...
            await self._validate_update(obj_id, obj_data, existing_obj, repository)
            update_dict = obj_data.model_dump(exclude_unset=True)
            updated_obj = await repository.update(obj_id, update_dict)
            if updated_obj is not None:
                await self._after_update(updated_obj, repository)
                await db.commit()
    ...
    async def delete(self, obj_id: int) -> bool:
        ...
        async with get_async_db_session() as db:
            ...
            await self._validate_delete(obj_id, existing_obj, repository)
            result = await repository.delete(obj_id)
            if result:
                await self._after_delete(obj_id, existing_obj, repository)
                await db.commit()
            return result

    # Template hooks: concrete services override only what they need

    async def _validate_create(self, obj_data: CreateSchemaType, repository: RepositoryType) -> None:
        """
    ...
    async def _after_create(self, created_obj: ModelType, repository: RepositoryType) -> None:
        """
    ...
    async def _validate_update(self, obj_id: int, obj_data: UpdateSchemaType, existing_obj: ModelType, repository: RepositoryType) -> None:
        """
    ...
    async def _after_update(self, updated_obj: ModelType, repository: RepositoryType) -> None:
        """
    ...
    async def _validate_delete(self, obj_id: int, existing_obj: ModelType, repository: RepositoryType) -> None:
        """
    ...
    async def _after_delete(self, obj_id: int, deleted_obj: ModelType, repository: RepositoryType) -> None:
        """
