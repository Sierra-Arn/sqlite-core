# app/db/sync_db/services/ml_metric.py
...
class MLMetricService(
    ...
    def _validate_create(self, obj_data: MLMetricRequestCreate, repository: MLMetricRepository) -> None:
        """
        ...
        repository : MLMetricRepository
            An active repository instance (provides access to the shared session via `repository.db`).
        ...
        ml_model = model_repo.get(obj_data.ml_model_id)
        if ml_model is None:
            raise ValueError(f"ML model with ID {obj_data.ml_model_id} does not exist")
        
        existing_metric = repository.get_by_model_id_and_name(
